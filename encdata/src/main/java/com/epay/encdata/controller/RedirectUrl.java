...
public class RedirectUrl {
	...
	public RedirectView redirectcallredirect(HttpServletRequest req) {
		...
		try {
			String value = myComponent.getMerchantId() + "|" + myComponent.getOperatingMode() + "|"
					+ myComponent.getMerchantCountry() + "|" + myComponent.getMerchantCurrency() + "|" + orderAmount
					+ "|" + myComponent.getOtherDetails() + "|" + myComponent.getSuccessUrl() + "|"
					+ myComponent.getFailUrl() + "|" + myComponent.getAggregatorId() + "|" + ordernumber + "|"
					+ ordernumber + "|" + myComponent.getPaymode() + "|" + myComponent.getAccessMedium() + "|"
					+ myComponent.getTransactionSource();
		...
		try {
			return new RedirectView(myComponent.getTransactionUrl()
					+ URLEncoder.encode(aggregatorHosted.getEncryptTrans(), "UTF-8") + "&EncryptbillingDetails="
					+ URLEncoder.encode(aggregatorHosted.getEncryptbillingDetails(), "UTF-8")
					+ "&EncryptshippingDetais="
					+ URLEncoder.encode(aggregatorHosted.getEncryptshippingDetais(), "UTF-8")
					+ "&EncryptpaymentDetails=" + aggregatorHosted.getEncryptpaymentDetails() + "&merchIdVal="
					+ aggregatorHosted.getMerchIdVal() + "&hiddenMerchantOrderNo="
					+ aggregatorHosted.getHiddenMerchantOrderNo() + "&neftRtgsMobileNumber="
					+ aggregatorHosted.getNeftRtgsMobileNumber() + "&neftRtgsEmailId="
					+ aggregatorHosted.getNeftRtgsEmailId() + "&neftChallanExpDt="
					+ aggregatorHosted.getNeftChallanExpDt() + "&cashCustomerName="
					+ aggregatorHosted.getCashCustomerName() + "&Submit=Proceed Transaction");
	...
	public String TransactionRequest(String MerchantOrderNo, String orderAmount1, String mkValue, String kkValue,
			String merchentid, String sucessurl, String failurl) {
		...
		try {
			...
			String value = "" + merchentid + "|" + myComponent.getOperatingMode() + "|"
					+ myComponent.getMerchantCountry() + "|" + myComponent.getMerchantCurrency() + "|" + orderAmount
					+ "|" + myComponent.getOtherDetails() + "|" + sucessurl + "|" + failurl + "|"
					+ myComponent.getAggregatorId() + "|" + ordernumber + "|" + ordernumber + "|"
					+ myComponent.getPaymode() + "|" + myComponent.getAccessMedium() + "|"
					+ myComponent.getTransactionSource();
		...
		try {
			return myComponent.getTransactionUrl() + URLEncoder.encode(aggregatorHosted.getEncryptTrans(), "UTF-8")
...
//added by saurabh
	...
	public String queryApiRequest(@RequestBody QueryRequest request) {
		...
		String encodedRequest = "queryRequest=" + queryRequest + "&aggregatorId=" + aggregatorId + "&merchantId="
				+ merchantId;
		...
		ResponseEntity<String> responseEntity = restTemplate.postForEntity(myComponent.getapiUrl(), httpEntity,
				String.class);
...
//added by saurabh
	...
	public String getTransactionMISAPIRequest(@RequestBody String request) {
		...
		try {
			...
			queryRequest = myComponent.getAggregatorId() + "|" + myComponent.getMerchantId() + "|"
					+ myComponent.getOperatingMode() + "|" + myComponent.getMerchantCountry() + "|"
			...
			RestTemplate restTemplate = new RestTemplate();
			ResponseEntity<String> responseEntity = restTemplate.postForEntity(myComponent.getTransactionMISAPI(),
...
//added by saurabh
	...
	public ResponseEntity<String> getQueryData(@RequestBody String statusdata) {
		...
		try {
			String meKey = mekKey.getMeK(myComponent.getMerchantId(), myComponent.getKkValue(),
		...
		ResponseEntity<String> responseBody = getQueryAPI1(encriptStatusRequest,
				checksum(jsondata1.toString(), myComponent.getChecksumMethod()), myComponent.getMerchantId());
	...
	public String getQueryAPI(String encdata, String cs, String merchantid) {
		...
		String encodedRequest = "encData=" + encdata + "&cs=" + cs + "&merchantCode=" + myComponent.getMerchantId();
	...
	public ResponseEntity<String> getQueryAPI1(String encdata, String cs, String merchantid) {
		...
		try {
			...
			ResponseEntity<String> response = restTemplate.postForEntity(myComponent.getStatusQueryAPI(), req,
					String.class);
